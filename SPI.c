...
{ 
  ...
  while ( i <= SSP_BUFSIZE ) 
  { 
/* to check the RXIM and TXIM interrupt, I send a block data at one time based on the FIFOSIZE(8). */ 
SSPSend( portnum, (uint8_t *)&src_addr[i], FIFOSIZE ); 
/* If RX interrupt is enabled, below receive routine can be also handled inside the ISR. */ 
...
dest_addr[i] = 0; 
  ...
  /* please note the first two bytes of WR and RD buffer is used for   commands and offset, so only 2 through SSP_BUFSIZE is used for data read,   write, and comparison. */ 
  SSP_SSELToggle( portnum, 0 ); 
...
initSSP1(); 
      ...
      LPC_GPIO0->FIOCLR = (1<<6); 
//**************************Read from buff 1    //Opcode for Read buff 1 
    ...
    //Send 1 dummy byte 
            ...
            printf("%c",SSP1exchangeByte(0x00)); 
            printf("%c",SSP1exchangeByte(0x00)); 
            printf("%c",SSP1exchangeByte(0x00)); 
            printf("%c",SSP1exchangeByte(0x00)); 
            printf("%c",SSP1exchangeByte(0x00)); 
            ...
            printf("%c",SSP1exchangeByte(0x00)); 
            printf("%c",SSP1exchangeByte(0x00)); 
            printf("%c",SSP1exchangeByte(0x00));           printf("\n******************************************************************************\n");   LPC_GPIO2->FIOSET = (1<<6); 
    for ( i = 0; i < 10000; i++ ); 
...
LPC_GPIO0->FIOCLR = (1<<6);//**************************Read from buff 1 
//Opcode for Read buff 1SSP1exchangeByte(0xd6); 
//Send 3 Bytes = 24 bits= 16 don't care bits + 8 buffer addr bits  
SSP1exchangeByte(0x00); 
SSP1exchangeByte(0x00); 
SSP1exchangeByte(0x00);//trial for buff1 
//Send 1 dummy byte   printf("\n",SSP1exchangeByte(0x00)); 
        printf("%c",SSP1exchangeByte(0x00)); 
        printf("%c",SSP1exchangeByte(0x00)); 
        printf("%c",SSP1exchangeByte(0x00)); 
      printf("%c",SSP1exchangeByte(0x00)); 
    ...
    printf("%c",SSP1exchangeByte(0x00)); 
        printf("\n******************************************************************************\n");LPC_GPIO2->FIOSET = (1<<6); 
for ( i = 0; i < 10000; i++ );//LPC_GPIO0->FIOCLR = (1<<6); 
        while(1); 
  //delayMs(0,100); 
 // printf("\n1\t%x", in); 
